...
echo "creating user"
./kcadm.sh create users -r selflet -s username=test -s enabled=true -s "attributes.agencyId=1" -s "attributes.agency=true" -s "attributes.tenant=false" -s "attributes.landlord=false" #'requiredActions=["UPDATE_PASSWORD"]'
...
echo "Adding test user to group Agency_Admin"
